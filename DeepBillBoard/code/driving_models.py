...
def Dave_v2(input_tensor=None, load_weights=True):
    ...
    model.add(Activation('relu'))
    #model.add(Dropout(p))
    model.add(Dense(10, init = ave2.hdf5') # it means the weights have been loaded
    model.compile(loss = 'mse', optimizer = 'Adam')normal_init, name = "dense_3"))
    model.add(Activation('relu'))
    model.add(Dense(1, init = normal_init, name = "dense_4"))
    model.add(Lambda(atan_layer, output_shape = atan_layer_shape, name = "prediction"))         #######
    model.load_weights('./models/dave2/d
    return model   
...
def Dave_v2(input_tensor=None, load_weights=True):
    ...
    model.add(Convolution2D(24, 5, 5, subsample=(2, 2), border_mode="valid", name='conv1_1'))
    model.add(ELU(name='elu1'))
    model.add(Convolution2D(36, 5, 5, subsample=(2, 2), border_mode="valid", name='conv2_1'))
    model.add(ELU(name='elu2'))
    model.add(Convolution2D(48, 5, 5, subsample=(2, 2), border_mode="valid", name='conv3_1'))
    model.add(ELU(name='elu3'))
    
    # 3x3 Convolutional layers with stride of 1x1
    model.add(Convolution2D(64, 3, 3, subsample=(1, 1), border_mode="valid", name='conv4_1'))
    model.add(ELU(name='elu4'))
    model.add(Convolution2D(64, 3, 3, subsample=(1, 1), border_mode="valid", name='conv4_2'))
    ...
    model.add(ELU(name='elu9'))
    
    # Output layer with tanh activation 
    model.add(Dense(1, activation='tanh', name='before_prediction'))
...
def Dave_v3(input_tensor = None,load_weights = False):
    ...
    model.add(convolutional.Convolution2D(16, 3, 3, input_shape=(100, 100, 3), activation='relu'))
    ...
    model.add(Lambda(One_to_radius, output_shape = atan_layer_shape, name = "before_prediction"))
    if load_weights:
